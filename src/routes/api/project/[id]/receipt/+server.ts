...
import { add_receipt } from '$lib/server/ledger';
...
export const POST: RequestHandler = async (event) => {
	...
	const b = (await event.request.json()) as { t: string; q: number; f?: string; x?: string };
	const r = await add_receipt(env, event.params.id, u.id, b);
	return json({ id: r.id });
};
